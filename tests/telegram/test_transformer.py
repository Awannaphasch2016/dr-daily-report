...
class TestResponseTransformer:
    ...
    def sample_state(self):
        ...
        return {
            ...
            'percentiles': {
                'rsi_percentile': {'percentile': 60},
                'price_percentile': {'percentile': 75}
            },
            'news': [
                {
...
''',
    ...
    def test_build_risk(self, transformer, sample_state):
        ...
        assert result is not None
        assert hasattr(result, 'risk_level')  # Changed from 'level'
        assert hasattr(result, 'risk_bullets')  # Changed from 'bullets'
