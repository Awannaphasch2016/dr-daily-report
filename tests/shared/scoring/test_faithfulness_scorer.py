...
class TestFaithfulnessScorer:
    ...
    def test_factual_correctness_correct(self):
        """Test factual correctness with correct facts"""
        narrative = """
        📖 เรื่องราวของหุ้นตัวนี้
        ...
        # Test factual_correctness key (implementation uses 'factual_correctness' not 'interpretation_accuracy')
        assert score.metric_scores['factual_correctness'] >= 80
    
    def test_claim_support_with_unsupported_claims(self):
        """Test claim support scoring with unsupported external claims"""
        narrative = """
        📖 เรื่องราวของหุ้นตัวนี้
        Uncertainty score 52/100 ซึ่งแสดงตลาดเสถียรมาก
        ATR 1.30% แสดงความผันผวนสูงมาก
        บริษัทประกาศขยายไปยุโรป  # External claim not in news
        ...
        # Test claim_support key (implementation uses 'claim_support' not 'interpretation_accuracy')
        assert score.metric_scores['claim_support'] < 100
        # Check that there are violations
        assert len(score.violations) > 0 or score.metric_scores['claim_support'] < 100
    ...
    def test_overall_score_calculation(self):
        ...
        # Updated weights to match implementation (35/25/20/15/5)
        expected = (
            score.metric_scores['numeric_accuracy'] * 0.35 +
            score.metric_scores['percentile_accuracy'] * 0.25 +
            score.metric_scores['news_citation_accuracy'] * 0.20 +
            score.metric_scores['factual_correctness'] * 0.15 +
            score.metric_scores['claim_support'] * 0.05
        )
