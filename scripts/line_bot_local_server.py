...
NOTE: This is a manual test server, not a pytest test.
Run this with: python3 tests/line_bot/test_line_local.py
"""
import pytest

# Skip this entire file when running pytest - it's a manual test server
pytestmark = pytest.mark.skip(reason="Manual test server - run with python directly")

from flask import Flask, request, jsonify
import json

# Lazy initialization to avoid import errors during pytest collection
app = Flask(__name__)
bot = None

def get_bot():
    """Lazy-load LineBot to avoid import errors during pytest collection"""
    global bot
    if bot is None:
...
def webhook():
    ...
    # Handle webhook
    result = get_bot().handle_webhook(body, signature)
...
def test():
    ...
    result = get_bot().handle_webhook(body, 'test_signature')
