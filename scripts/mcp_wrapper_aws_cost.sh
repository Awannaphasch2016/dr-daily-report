#!/bin/bash
# Wrapper script for AWS Cost Management MCP Server with Doppler environment variables
...
# Run the Cost Management MCP server directly (not wrapped in doppler)
exec uvx awslabs.cost-management-mcp-server@latest "$@"
