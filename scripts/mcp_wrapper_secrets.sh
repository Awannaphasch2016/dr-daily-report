#!/bin/bash
# Wrapper script for AWS Secrets Manager MCP Server with Doppler environment variables
...
# Run the Secrets Manager MCP server directly (not wrapped in doppler)
exec uvx awslabs.secrets-manager-mcp-server@latest "$@"
