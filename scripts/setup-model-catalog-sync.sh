#!/usr/bin/env bash
# =============================================================================
# setup-model-catalog-sync.sh — Provision model catalog sync Lambda + schedule
#
...
#
# Usage:
#   ./scripts/setup-model-catalog-sync.sh              # provision all resources
#   ./scripts/setup-model-catalog-sync.sh --cleanup    # tear down all resources
#
# Prerequisites:
...
FUNCTION_NAME="${PROJECT}-model-catalog-sync-${ENV}"
LOG_GROUP="/aws/lambda/${FUNCTION_NAME}"
ALARM_NAME="${FUNCTION_NAME}-errors-${ENV}"
SCHEDULE_NAME="${FUNCTION_NAME}"
HANDLER="src.alerting.model_catalog_sync.lambda_handler"
TIMEOUT=60
MEMORY=256
...
if [[ "${1:-}" == "--cleanup" ]]; then
    echo "============================================"
    echo "  CLEANUP: Removing model catalog sync resources"
    echo "============================================"
...
ENV_VARS="{\"Variables\":{\"TZ\":\"Asia/Bangkok\",\"AURORA_HOST\":\"${AURORA_HOST}\",\"AURORA_USER\":\"${AURORA_USER}\",\"AURORA_PASSWORD\":\"${AURORA_PASSWORD}\",\"AURORA_DATABASE\":\"${AURORA_DATABASE}\",\"ENVIRONMENT\":\"${ENV}\"}}"
...
if aws lambda get-function --function-name "$FUNCTION_NAME" --region "$REGION" &>/dev/null; then
    ...
    aws lambda create-function \
        ...
        --tags "Project=${PROJECT},Environment=${ENV},App=shared,ManagedBy=aws-cli,Component=model-catalog-sync" \
        --region "$REGION" > /dev/null
    log "  Created"
fi
...
# ---------------------------------------------------------------------------
# Step 5: EventBridge Scheduler (daily 4 AM Bangkok — 1h before pipeline)
# ---------------------------------------------------------------------------
log "Step 5: EventBridge schedule (4 AM Bangkok daily)..."

# Create scheduler role if needed
SCHEDULER_ROLE_NAME="${PROJECT}-scheduler-model-catalog-${ENV}"
...
log "  Schedule configured: daily 4 AM Bangkok"
...
aws cloudwatch put-metric-alarm \
    ...
    --alarm-description "Model catalog sync Lambda errors" \
...
# ---------------------------------------------------------------------------
echo "============================================"
echo "  Model Catalog Sync — Provisioned!"
echo "============================================"
echo ""
echo "  Lambda:    ${FUNCTION_NAME}"
echo "  Alias:     ${FUNCTION_NAME}:live (v${LATEST_VERSION})"
echo "  Schedule:  ${SCHEDULE_NAME} (daily 4 AM Bangkok)"
echo "  Log group: ${LOG_GROUP}"
echo "  Alarm:     ${ALARM_NAME}"
echo ""
echo "MANUAL TEST:"
echo "  aws lambda invoke --function-name ${FUNCTION_NAME} \\"
echo "    --payload '{}' /tmp/model-catalog-result.json --region ${REGION}"
echo "  cat /tmp/model-catalog-result.json | python3 -m json.tool"
echo ""
