#!/bin/bash
# Wrapper script for AWS Lambda MCP Server with Doppler environment variables
...
# Run the Lambda MCP server directly (not wrapped in doppler)
exec uvx awslabs.lambda-mcp-server@latest "$@"
