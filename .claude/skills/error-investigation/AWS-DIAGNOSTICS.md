...
aws cloudwatch get-metric-statistics \
  ...
  --dimensions Name=TableName,Value=dr-daily-report-telegram-jobs-dev \
  --start-time $(date -u -d '1 hour ago' +%Y-%m-%dT%H:%M:%S) \
  --end-time $(date -u +%Y-%m-%dT%H:%M:%S) \
  --period 300 \
  --statistics Sum
...
aws s3api get-bucket-policy \
  --bucket dr-daily-report-data-dev \
...
aws iam simulate-principal-policy \
  ...
  --resource-arns arn:aws:s3:::dr-daily-report-data-dev/*
```
...
aws s3 ls s3://dr-daily-report-data-dev/reports/
...
aws cloudwatch get-metric-statistics \
  ...
  --dimensions Name=BucketName,Value=dr-daily-report-data-dev \
  --start-time $(date -u -d '1 hour ago' +%Y-%m-%dT%H:%M:%S) \
  --end-time $(date -u +%Y-%m-%dT%H:%M:%S) \
  --period 300 \
  --statistics Sum
...
aws rds describe-db-clusters \
  --db-cluster-identifier dr-daily-report-dev \
...
aws cloudwatch get-metric-statistics \
  ...
  --dimensions Name=DBClusterIdentifier,Value=dr-daily-report-dev \
  --start-time $(date -u -d '1 hour ago' +%Y-%m-%dT%H:%M:%S) \
  --end-time $(date -u +%Y-%m-%dT%H:%M:%S) \
  --period 60 \
  --statistics Maximum
...
aws rds download-db-log-file-portion \
  --db-instance-identifier dr-daily-report-dev-instance-1 \
...
QUEUE_URL=$(aws sqs get-queue-url \
  --queue-name dr-daily-report-worker-dev \
